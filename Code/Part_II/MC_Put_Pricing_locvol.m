...
CPU_naive, CPU_antithetic, CPU_control] = MC_Put_Pricing_locvol...
( Smin, Smax, K, r_0, sigma_0, T, err )
% MC_Put_Pricing_locvol -  computes for Monte Carlo simulations of a put 
% with a local volatility model the number of paths required to
% ensure a pricing error less than the input "err". The price of the
...
% for the naive Monte Carlo method, and with antithetic variance reduction
% and control variates. It will also produce a plot of the put prices for
% these various pricing methods and a plot of the number of paths needed to
% be simulated to ensure a pricing error less than "err".
...
for i=1:length(S)
[price_naive(i), volatility_naive(i)] = MC_Put_local_vol( S(i), ...
K, r_0, sigma_0, T, Npath );
end
...
= MC_Put_local_vol( S(i), K, r_0, sigma_0, T, Npath_required_naive );
end
...
MC_Put_local_vol_antithetic( S(i), K, r_0, sigma_0, T, Npath );
end
...
CPU_antithetic_min(i)] = MC_Put_local_vol_antithetic( S(i), ...
K, r_0, sigma_0, T, Npath_required_antithetic );
end
...
% minimum pricing error
for i=1:length(S)
[price_control(i), volatility_control(i)] = MC_Put_local_vol_control ...
( S(i), K, r, sigma, r_0, sigma_0, T, Npath );
end
...
CPU_control_min(i)] = MC_Put_local_vol_control( S(i), K, r, ...
sigma, r_0, sigma_0, T, Npath_required_control );
end
...
figure
hold on 
plot(S, price_naive_min,'k*')
plot(S, price_antithetic_min,'r*')
plot(S, price_control_min,'gd')
hold off
...
figure
hold on
plot(S, Npath_required_naive_range)
plot(S, Npath_required_antithetic_range)
plot(S, Npath_required_control_range)
xlabel("Spot price","Fontsize",14)
